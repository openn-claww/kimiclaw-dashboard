#!/usr/bin/env python3
"""
Redeem Blazers vs Suns winning position
Market resolved: Trail Blazers won (outcome 0)
Our position: NO on Suns (winning position)
"""
...
MARKET = {
    ...
    # NO token ID (Suns lost, so NO is winning)
    'no_token_id': '94005336293241469024004520025094687913386243374672117887301233895307085665296',
    'yes_token_id': '61208409487579219570477960039139418649842262205786179762630480653396721246836',
    'winning_index': 2,  # NO = index 2
}
...
# Check NO token balance (winning position)
no_token_id = int(MARKET['no_token_id'])
balance = ctf.functions.balanceOf(address, no_token_id).call()
print(f"NO Token Balance: {balance / 1e6:.6f} USDC.e")

if balance == 0:
    print("⚠️  No winning tokens found - may already be redeemed")
    sys.exit(0)

print(f"🎯 Found winning position! Attempting redemption...")
