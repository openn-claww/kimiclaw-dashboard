#!/usr/bin/env python3
"""
FIXED WALLET BOT - Uses alternative API endpoints
"""
...
def get_crypto_markets():
    """Try to find crypto up/down markets from various sources"""
    markets = []
    
    # Try Gamma API with different params
    try:
        resp = requests.get(
            "https://gamma-api.polymarket.com/markets?active=true&closed=false&archived=false&limit=200",
            timeout=3
        )
        if resp.status_code == 200:
            ...
            for m in data.get('markets', []):
                ...
                if any(x in q for x in ['bitcoin', 'btc', 'ethereum', 'eth', 'solana', 'sol', 'xrp', 'ripple']):
                    if any(x in q for x in ['up', 'down', 'higher', 'lower', 'above', 'below']):
                        markets.append({
                            'id': m.get('id'),
                            'slug': m.get('slug'),
...
def evaluate_markets():
    ...
    if current_time - last_trade_time < 30:  # Min 30s between trades
        return
    ...
    if not markets:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] No crypto markets found via API")
        return
    
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Found {len(markets)} crypto markets")
    ...
    for m in markets:
        try:
            ...
            if len(prices_list) == 2:
                ...
                # Check edge (need velocity from WebSocket)
                coin = None
                q = m['question'].lower()
...
def on_message(ws, message):
    ...
    try:
        ...
        if symbol and price:
            ...
            # Evaluate markets on each price update
            evaluate_markets()
...
def on_open(ws):
    print(f"[{datetime.now().strftime('%H:%M:%S')}] FIXED Wallet 1 Bot: WebSocket CONNECTED!")
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Searching for crypto markets...")

print("="*70)
print("FIXED WALLET 1 BOT - Alternative API Approach")
print("="*70)

# Test API first
test_markets = get_crypto_markets()
print(f"Initial scan: Found {len(test_markets)} crypto markets")
for m in test_markets[:3]:
