#!/usr/bin/env python3
"""
BACKTEST: V4 vs V6 (Proper Comparison)
"""
...
def run_backtest(name, use_kelly=False, n_samples=3000):
    ...
    for _ in range(n_samples):
        ...
        if use_kelly:
            ...
            if total_trades >= 15:
                ...
                if win_rate > 0.5 and b > 0.8:
                    ...
                    kelly_f = max(0, min(kelly_f * 0.5, 0.10))  # Half-Kelly, max 10%
                else:
                    kelly_f = 0.02  # Conservative default
            else:
                kelly_f = 0.02  # Bootstrap mode
        ...
        # Simulate trade outcome (V4+ has better edge)
...
# Run both backtests
print("="*80)
print("BACKTEST: V4 vs V6 (Kelly)")
print("="*80)
print()

print("Running V4 (Fixed 5% sizing)...")
v4_result = run_backtest("V4: Fixed 5%", use_kelly=False)

print("Running V6 (Kelly Criterion)...")
v6_result = run_backtest("V6: Kelly Criterion", use_kelly=True)

results = [v4_result, v6_result]
...
trade_diff = v6['trades'] - v4['trades']
...
print(f"Trades:  V4={v4['trades']} vs V6={v6['trades']} → {trade_diff:+d}")
print()

if v6['return_pct'] > v4['return_pct']:
    print("✅ V6 (Kelly) IMPROVED performance!")
elif v6['return_pct'] < v4['return_pct'] * 0.8:
    print("❌ V6 (Kelly) HURT performance significantly")
else:
    print("⚠️  V6 (Kelly) had similar performance")

# Save
with open('/root/.openclaw/workspace/v4_vs_v6_final.json', 'w') as f:
    json.dump(results, f, indent=2)

print()
print("Results saved to v4_vs_v6_final.json")
